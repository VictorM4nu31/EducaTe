<x-layouts::app title="Editar Docente">
    <div class="container mx-auto py-6 max-w-2xl">
        <div class="mb-8">
            ...
            <p class="text-neutral-500 dark:text-neutral-400">Actualizar información de {{ $teacher->name }}</p>
        </div>
        ...
        <div class="bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800 p-6 shadow-sm">
            <form action="{{ route('admin.teachers.update', $teacher) }}" method="POST" class="space-y-6">
                ...
                <!-- Nombre -->
                <flux:input label="Nombre Completo" name="name" required :value="old('name', $teacher->name)" />

                <!-- Email -->
                <flux:input label="Correo Electrónico" type="email" name="email" required :value="old('email', $teacher->email)" />

                <!-- RFC -->
                <flux:input label="RFC" name="rfc" :value="old('rfc', $teacher->rfc)" />
